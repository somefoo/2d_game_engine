...
renderer::renderer(const int width, const int height) : width_(width), height_(height), framebuffer_(height * width * sizeof(unsigned char), 0)
{
    //empty
}
...
//    @TODO what about black? This is ugly xD
    #define LOOP_CONTENT \
            ...
            unsigned char *pixel = &framebuffer_[(p.x - c.x + x) + width_ * (p.y - c.y + y)];\
            *pixel = s->get_pixel(xx_v, yy);
    int yy = 0;
    if(!flip_x && !flip_y)
        for(int y=0;y<h;++y)    {int xx = 0; for(int x=0;x<w;++x) {LOOP_CONTENT} yy++;}
    else if(flip_x && !flip_y)
        for(int y=0;y<h;++y)    {int xx = 0; for(int x=w-1;x>0;--x) {LOOP_CONTENT} yy++;}
    else if(!flip_x && flip_y)
        for(int y=h-1;y>0;--y)  {int xx = 0; for(int x=0;x<w;++x) {LOOP_CONTENT} yy++;}
    else
        for(int y=h-1;y>0;--y)  {int xx = 0; for(int x=w-1;x>0;--x){LOOP_CONTENT} yy++;}
...
unsigned char renderer::create_color(unsigned char r, unsigned char g, unsigned char b)
{
    assert(r < 8 && g < 8 && b < 4);
    return (r << 5) + (g << 2) + (b);
}