...
game_object::game_object(sprite* s){
    set_sprite(s);
}
...
void game_object::set_depth(short depth){
    depth_ = depth;
}
...
void game_object::set_sprite(sprite* s){
    sprite_ = s;
}
...
sprite const* game_object::get_sprite() const{
    return sprite_;
}
...
bool game_object::hit(const ivec2 location) const{
    ...
    if(sprite_->get_pixel(relative_position.x, relative_position.y) != TRANSPARENT){
        return true;
    } 
    return false;
}
...
bool game_object::hit_ray_bounding_box(const ivec2 origin, const ivec2 direction, int *dist) const{
    (void) direction;
    ivec2 relative_min = position_;
    ivec2 relative_max = position_ + ivec2(sprite_->get_width(), sprite_->get_height());
