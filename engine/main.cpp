...
void *start_update(void *run_state) {
  ...
  while (!state->exited) {
    std::this_thread::sleep_for(
        std::chrono::milliseconds(33 - deltaTime / 1000));
    auto t1 = Clock::now();
    ...
    if (counter == 30) {
      counter++;
      std::cout << "fps: " << 1000000 / ((float)msCounter / (float)counter)
                << "fps" << std::endl;
      counter = 0;
      msCounter = 0;
    }
...
int main(int argc, char **argv) {
  ...
  SDL_Texture *sdlTexture =
      SDL_CreateTexture(sdlRenderer, SDL_PIXELFORMAT_RGB332,
  ...
  while (!run_state.exited) {
    ...
    if (run_state.allow_draw && m.try_lock()) {
      ...
      SDL_UpdateTexture(sdlTexture, NULL, g.get_framebuffer(),
                        sizeof(char) * WIDTH);
      // SDL_RenderClear(sdlRenderer);
